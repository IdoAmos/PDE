...
class ConvSineLayer(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, kernel_size, bias=True,
        ...
        self.in_features = kernel_size * kernel_size * in_ch
        self.conv = nn.Conv2d(in_ch, out_ch, kernel_size, padding=(kernel_size - 1) // 2)
    ...
    def forward_with_intermediate(self, input):
        # For visualization of activation distributions
        intermediate = self.omega_0 * self.conv(input)
        return torch.sin(intermediate), intermediate


class ConvSiren(nn.Module):
    def __init__(self, in_features, num_ch, hidden_layers, out_features
                 , nhood=3, outermost_linear=False, first_omega_0=30, hidden_omega_0=30.):
        super().__init__()
        self.ch = num_ch
        self.nhood = nhood
        self.num_layers = hidden_layers + 2  # 2 for first and last
        self.net = []
        self.net.append(SineLayer(in_features, num_ch[0] * nhood * nhood,
                                  is_first=True, omega_0=first_omega_0))

        for i in range(hidden_layers):
            self.net.append(ConvSineLayer(num_ch[i], num_ch[i + 1], kernel_size=3,
                                          is_first=False, omega_0=hidden_omega_0))
        if outermost_linear:
            final_linear = nn.Linear(num_ch[-1] * nhood * nhood, out_features)

            with torch.no_grad():
                final_linear.weight.uniform_(-np.sqrt(6 / num_ch[-1] * nhood * nhood) / hidden_omega_0,
                                             np.sqrt(6 / num_ch[-1] * nhood * nhood) / hidden_omega_0)

            self.net.append(final_linear)
        else:
            self.net.append(SineLayer(num_ch[-1] * nhood * nhood, out_features,
                                      is_first=False, omega_0=hidden_omega_0))
    ...
    def forward(self, coords):
        ...
        for i, layer in enumerate(self.net):
            if i == 0:
                h = layer(coords)
                h = h.view(-1, self.ch[0], self.nhood, self.nhood)
            elif i == self.num_layers - 2:
                h = layer(h)
                h = h.view(-1, self.ch[-1] * self.nhood * self.nhood)
            else:
                h = layer(h)
        return h, coords
    ...
    def forward_with_activations(self, coords, retain_grad=False):
        ...
        activations['input'] = x
        for i, layer in enumerate(self.net):
            if isinstance(layer, (ConvSineLayer, SineLayer)):
                if i == self.num_layers - 2:
                    x, intermed = layer.forward_with_intermediate(x)
                    x = x.view(-1, self.ch[-1] * self.nhood * self.nhood)
                elif i == 0:
                    x, intermed = layer.forward_with_intermediate(x)
                    x = x.view(-1, self.ch[0], self.nhood, self.nhood)
                else:
                    x, intermed = layer.forward_with_intermediate(x)
